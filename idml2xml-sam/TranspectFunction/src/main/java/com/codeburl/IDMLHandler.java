...
public class IDMLHandler {
  ...
  public String handleEvent(S3Put event) throws IOException {
    ...
    final String source_idml_fn = downloadInputIDML(input_bucket, input_key);
    final String output_fn = invokeXProc(source_idml_fn);
    logger.info("HUB XML output sent to " + output_fn);
    final String output_bucket= System.getenv("OUTPUT_BUCKET");
    final String result_arn = uploadOutputXML(output_bucket, output_fn, input_key);
    return result_arn;
  }

  private String invokeXProc(String source_idml_fn) throws IOException {
  ...
  private String downloadInputIDML(String bucket, String key) {
    UUID uuid = UUID.randomUUID();
    final String source_idml_fn = "/tmp/" + uuid.toString() + ".idml";
		try {
			S3Object o = s3.getObject(bucket, key);
			S3ObjectInputStream s3is = o.getObjectContent();
      logger.info("Downloading IDML file from S3 (arn:aws:s3:::" + bucket + "/" + key + ") to " + source_idml_fn);
			FileOutputStream fos = new FileOutputStream(new File(source_idml_fn));
    ...
    return source_idml_fn;
  }

  private String uploadOutputXML(String output_bucket, String output_fn, String input_key) {
    UUID uuid = UUID.randomUUID();
    final String basename = FilenameUtils.getBaseName(input_key);
    final String output_key = basename + ".xml";
